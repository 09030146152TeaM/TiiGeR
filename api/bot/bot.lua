...
bot_token = "490443760:AAFUwKlykGCcHBAQieITSJPiK8kpeWDxhI4" -- در اینجا توکن ربات api  را قرار دهید
send_api = "https://api.telegram.org/bot"..bot_token
sudo_id = 97476012 -- ایدی سودوی اصلی را قرار دهید
MaTaDoRch = '@MaTaDoRsupporT' -- یوزرنیم کانال خود را قرار دهید
MaTaDoRby = '@SaberTiger' -- یوزرنیم خود را قرار دهید
require('./bot/methods')
require('./bot/utils')
require('./libs/JSON')
...
function create_config( )
	...
	if server_user:match("^root$") then
		MaTaDoRTeaM = '/root/TiiGeR/cli'
	elseif not server_user:match("^root$") then
		TiiGeRTeaM = '/home/'..server_user..'/TiiGeR/cli'
	end
  ...
  config = {
    ...
    sudo_users = {490443760,474987896,97476012}, -- در اینجا آیدی ربات api و cli  و سودو ها را وارد کنید
    admins = {},
    disabled_channels = {},
    moderation = {data = ''..TiiGeRTeaM..'/data/moderation.json'},
  }
